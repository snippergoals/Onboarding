...
class SurveyController extends Controller
{
    ...
    {
        $section = QuestionSection::where('id', $request['section']['id'])->first();
        $section['name'] = $request['section']['name'];
        $section->save();

        return response('Update Successful.', 200);
    }
    public function updateSurveyOrder(Request $request)
    {
        foreach ($request->sections as $dragged_section) {
            ...
            foreach ($dragged_section['questions'] as $dragged_question) {
                ...
                $question['question_section_id'] = $dragged_section['id'];
                $question->save();  
            }
        }
        return response('Update Successful.', 200);
