...
class DirectoryHelper
{
     ...
     * @return mixed
     */
    public function getOrCreateUserLogoDirectory($slug)
    {
        $userDirectory = $this->getOrCreateUserDirectory($slug);

        $assetDirectory = $this->getDirectory($userDirectory['path'] . "/", 'logo');

        if (!$assetDirectory) {
            Storage::cloud()->makeDirectory($userDirectory['path'] . "/". 'logo');
            $assetDirectory = $this->getDirectory($userDirectory['path'] . "/", 'logo');
        }
