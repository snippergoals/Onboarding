    ...
    <div class="col-md-12">
      <div class="card card-default">
        <div class="card-header">
          <div class="pull-left">
            <div style="font-size: x-large;">{{ $user->project_name }} <small class="text-black-50"><small>{{$user->company}}</small></small></div>
          ...
          <div class="card-body">
            <p>
              ...
              @elseif(isset($assets->value))
                    Link: <a target="_blank" href="{{$assets->value}}">{{$assets->value}}</a>
              @endif
            </p>
          ...
          <div class="card-body">
            <p>
              @if(isset($logos->folder))
                  <a target="_blank" href="https://drive.google.com/drive/folders/{{$logos->folder}}">
                    <i class="fa fa-folder-open"></i>
                    Logos Folder
                  </a>
                {{-- @endif --}}
              @elseif(isset($logos->value))
                    Link: <a target="_blank" href="{{$logos->value}}">{{$logos->value}}</a>
              @endif
            </p>
            ...
            @if(!is_null($logos))
              @if(!$logos->is_reviewed)
                <button
                        data-toggle="modal" data-target="#logos-modal"
                        type="button"
                        class="btn btn-sm btn-primary">
                  Review
                </button>
            ...
            @endif
              ...
              <div class="modal fade" id="logos-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg" role="document">
                  <form method="POST" action="{{route('admin.review-logos', [$user, $logos])}}">
                    {{csrf_field()}}
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Feedback</h5>
                      ...
                      <div class="modal-body">
                        <div class="form-group{{ $errors->has('feedback') ? ' has-error' : '' }}">
                          <textarea name="feedback" id="feedback" cols="30" rows="10" class="form-control">{{old('feedback',isset($logos->feedback)?$logos->feedback:'')}}</textarea>
                          @if ($errors->has('feedback'))
                          <span class="help-block">
                              <strong>{{ $errors->first('feedback') }}</strong>
                          </span>
            ...
            <p>
              @if(isset($prototype->folder))
                ...
                @elseif(isset($prototype->value))
                   Link: <a target="_blank" href="{{$prototype->value}}">{{$prototype->value}}</a>
                @endif
            </p>
  ...
  <div class="container">
    ...
    <div class="row justify-content-center">
      <div class="col-md-12">
        <div class="card card-default">
          <div class="card-header">
            <span class="card-title">Keybase</span>
          </div>
          <div class="row card-body">
            <div class="col-md-6">
              @if (!is_null($user->keybase))
                <pre>{{ $user->keybase }}</pre>
                <p class="font-italic">Once you copy & decrypt the info above click the button below to destroy the data. Warning, once you click that button its gone forever. You have been warned.</p>
                <p><a href="{{ route('admin.remove_keybase', [$user]) }}" class="btn btn-primary">Nuke It</a></p>
              @else
                <p>Waiting on client to upload.</p>
              @endif
            </div>
          </div>
        </div>
        ...
        <div class="col-md-12">
          <div class="card card-default">
            ...
            <div class="card-body">
              ...
              <p><strong>Deal Type:</strong> {{ $user->deal_type }}</p>
              <p><strong>Platform:</strong> {{ $user->platform }}</p>
              <p><strong>Salesperson:</strong> {{ $user->salesperson }}</p>
