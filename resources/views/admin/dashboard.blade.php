                ...
                @foreach($users as $user)
                  <tr>
                    <td scope="row" class="text-left">
                      <div class="mt--25">
                        <a href="{{route('admin.customer.show', $user)}}" class="pt-3">
                          {{$user->project_name}}
                        </a>
                      </div>
                      <div class="mt--5">
                        <a href="{{route('admin.customer.show', $user)}}" class="text-black-50">
                    ...
                    <td>
                      <p>
                        @if(!is_null($user->logo))
                          @if($user->logo->is_reviewed)
                            @if($user->logo->is_complete)
                              <span class="btn btn-sm btn-outline-success">Approved</span>
                            @else
                              <span class="btn btn-sm btn-outline-danger">Declined</span>
                            @endif
                    ...
                    <td>
                      <p>
                        @if(!is_null($user->keybase))
                          <span class="btn btn-sm btn-outline-success">Added</span>
                        @else
                          <span class="btn btn-sm btn-outline-secondary">Not Added</span>
                        @endif
                      </p>
                    </td>
                    @foreach(config('onboard.services') as $service)
